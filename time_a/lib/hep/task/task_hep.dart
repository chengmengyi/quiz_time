...
class TaskHep{
  ...
  Future<List<TaskBean>> getTaskListByPayTypeAndChooseMoney(PayType payType,int chooseMoney)async{
    //list only one
    var list = await SqlHepA.instance.queryTaskRecordByPayTypeAndChooseMoney(payType,chooseMoney);
    if(list.isEmpty){
      return [];
    }
    ...
    if((taskRecord.completedNum??0)<(taskRecord.totalNum??0)||(taskRecord.signedNum??0)<(taskRecord.signTotalNum??0)||currentTaskTypeIndex==_taskTypeList.length-1){
      ...
      var newTaskRecord = await SqlHepA.instance.refreshTaskRecordByTaskType(payType, chooseMoney, nextTaskType);
  ...
  String _getTaskTitleByType(TaskRecord taskRecord){
    var data = ValueHepA.instance.getTaskByTitle(taskRecord.taskType??"").data;
